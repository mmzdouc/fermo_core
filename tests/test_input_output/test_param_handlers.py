...
def test_init_feature_filtering_parameters_valid():
    ...
    assert isinstance(i, FeatureFilteringParameters)
...
def test_init_feature_filtering_parameters_fail():
    ...
    with pytest.raises(ValueError):
        FeatureFilteringParameters(
            **{"filter_rel_area_range_min": 1.0, "filter_rel_area_range_max": 0.1}
        )
...
def test_init_adduct_annotation_parameters_valid():
    ...
    assert isinstance(i, AdductAnnotationParameters)
...
def test_init_spec_sim_network_cosine_parameters_valid():
    ...
    assert isinstance(i, SpecSimNetworkCosineParameters)
...
def test_init_speclib_parameters_fail():
    ...
    with pytest.raises(ValueError):
        SpecLibParameters(
            **{"dirpath": "tests/test_data/spec_lib/", "format": "asdfas"}
        )


def test_outputparams_valid():
    i = OutputParameters(directory_path=Path("tests/test_data/"))
